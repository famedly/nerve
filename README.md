...
| `DATAPOINT_SERVER` | *(unset)* | TCP endpoint (`host:port`) of a [datapoint collector](datapoint/) to which a fixed-size 32-byte record is sent for every received message. The record is `timestamp_us (u64) \| sender (u32) \| receiver (u32) \| serial (u64) \| delivery_us (u32) \| media_size_bytes (u32)`, all little-endian. Fields that exceed their type are clamped to the maximum. `sender` / `receiver` are 1-based account numbers from `ACCOUNT_LIST` (0 when unknown). |
| `ACCOUNT_LIST` | *(unset)* | Either a space-delimited list of MXIDs, or a path to a file containing one MXID per line. Used to map MXIDs to compact 1-based account numbers for `DATAPOINT_SERVER` submissions; MXIDs not in the list are reported as account `0`. |
